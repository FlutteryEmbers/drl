...
train_envs = {
    ...
    '16_16_2base':SimpleNamespace(**{'map_size': 16*16,'conv_linear':6*6,'latent_size':256,'map_path':'maps/16x16/TwoBasesBarracks16x16.xml','num_steps':512,'max_version':int(5e4)}),
    '16_16_b':SimpleNamespace(**{'map_size': 16*16,'conv_linear':6*6,'latent_size':256,'map_path':'maps/16x16/basesWorkers16x16B.xml','num_steps':512,'max_version':int(5e4)}),
    '16_16_melee':SimpleNamespace(**{'map_size': 16*16,'conv_linear':6*6,'latent_size':256,'map_path':'maps/16x16/melee16x16Mixed12.xml','num_steps':512,'max_version':int(5e4)}),
...
def init():
    ...
    train_config = utils.load_config('assets/configs/microrts_16_16_seed10_loss0.yaml')
    ...
    comment = "_PPOMicroRTSSharePAE_condition_" + current_env_name + "_seed{}_loss{}_".format(train_config['seed'], train_config['pg_loss_type']) + train_config['tensorboard_comment']
...
class PPOMicroRTSSharePAEConditionNet(AlgoBase.AlgoBaseNet):
    ...
    """
    def __init__(self):
        super(PPOMicroRTSSharePAEConditionNet,self).__init__()
...
class PPOMicroRTSSharePAEConditionUtils(AlgoBase.AlgoBaseUtils):
    pass
        
class PPOMicroRTSSharePAEConditionAgent(AlgoBase.AlgoBaseAgent):
    """
    ...
    """
    def __init__(self,sample_net:PPOMicroRTSSharePAEConditionNet,model_dict,is_checker=False):
        super(PPOMicroRTSSharePAEConditionAgent,self).__init__()
    ...
    def sample_env(self, check=False):  
        ...
        while len(self.exps_list[0]) < self.num_steps:
            ...
            next_obs, rs, done_n, env_infos = self.env.step(action)
            ...
            for i in range(self.num_envs):
                ...
                if check:
                    if done_n[i]:
                        if env_infos[i]['raw_rewards'][0] > 0:
...
class PPOMicroRTSSharePAEConditionCalculate(AlgoBase.AlgoBaseCalculate):
    """
    ...
    """
    def __init__(self,share_model:PPOMicroRTSSharePAEConditionNet,model_dict,calculate_index):
        super(PPOMicroRTSSharePAEConditionCalculate,self).__init__()
        self.model_dict = model_dict
        ...
        self.calculate_net = PPOMicroRTSSharePAEConditionNet()
...
if __name__ == "__main__":
    ...
    train_net = PPOMicroRTSSharePAEConditionNet()
    ...
    for _ in range(MAX_VERSION):
        ...
        print("version:",model_dict[0],"mean_step_rewards:",infos['mean_rewards'])
